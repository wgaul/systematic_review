...
**TODO: As of 13 Sep, I have removed columns from the 'systematic_review_coded_results.ods' spreadsheet and changed a bunch of column names.  Need to revise this script to make sure that this all still runs appropriately (e.g. that columns are not accessed by number position, etc).**
...
# TODO replace this with Ellie's results
...
mult_coded_wg <- wg[which(wg$title %in% cp$title), 
                      which(colnames(wg) %in% colnames(cp))]
# order rows
...
if(any(mult_coded_wg$title %nin% cp$title) | 
   any(cp$title %nin% mult_coded_wg$title)) {
  warning("cp and mult_coded_wg don't have the same titles. One or both of those data frames will be subsetted.")
  mult_coded_wg <- mult_coded_wg[which(mult_coded_wg$title %in% cp$title), ]
  cp <- cp[which(cp$title %in% mult_coded_wg$title), ]
}

## make sure df rows and cols are in the same order
if(!identical(colnames(cp), colnames(mult_coded_wg))) {
  stop("Columns in cp and mult_coded_wg must be identical and in the same order. They must be in the same order for creating the variable data frames that will be used to calculate Krippendorf's alpha. ")
}
```
...
if(colnames(cp)[1] != "title" | colnames(mult_coded_wg)[1] != "title") {
  stop("The first column of cp and mult_coded_wg must be the study title")}

for(i in 2:ncol(cp)) {
  post_correct_list[[i-1]] <- data.frame(matrix(
    data = c(cp[, i][[1]], mult_coded_wg[, i][[1]]), 
...
kripp_sum_cap <- figs(name = "kripp_sum_cap", 
                      paste0("Reader agreement for ", 
                             ...
                             length(which(mult_coded_wg$title %nin% 
                                            calibration_set)), 
...
One reader has coded `r nrow(wg)` articles and a second reader has coded `r nrow(cp_orig)` of those. 
...
alpha_df_all <- data.frame(
  ...
  n_wg_T = sapply(names(kripp_all_pre_correct), 
                  FUN = function(x, df = mult_coded_wg_orig) {
                    df <- data.frame(df)
...
## This calculates the proportion of TRUE values using wg's 
# codings for each variable.  Even though we are interested in kripp's alpha
# for double-coded articls, I use the post-correction TRUEs as the best 
# estimate of the
...
prob_of_values <- lapply(
  wg[, which(colnames(wg) %nin% c("title", "qualifies", "authors", 
                                  "publication", "year", "keywords", 
                                  "institution.of.first.author", 
                                  "institution.of.last.author", 
                                  "proximate.data.source", 
                                  "country.of.study", "spatial.extent.of.study", 
                                  "start.year", "end.year", 
                                  "taxonomic.group",
                                  "grid.resolution", 
                                  "prediction.performance.measure", 
                                  "analysis.method", "predictor.variables", 
                                  "methodology.focus", 
                                  "coding.DONE", 
                                  "temp_extent", "log_years"))], 
...
for(i in 1:nrow(prob_of_values)) {
  ...
  prob_of_values$kripp_all[i] <- kripp_all[[which(
    names(kripp_all) == prob_of_values$variable[i])]]$value
}
...
attr(dt_pd_df, "data types") <- c(
  ...
  "data.type...life.stage", 
  "data.type...voucher.available")
attr(dt_pd_df, "study paradigm") <- c("community.question", 
                                      "individual.species.question")
...
qt_df <- select(wg, "title", "year", "methodology.development.or.analysis", 
                "distribution", "abundance", "phenology", 
...
```{r}
qt_df <- select(wg, "title", "year", "methodology.development.or.analysis", 
                "individual.species.question", "community.question", 
                "compiled.individual.species.question", "distribution", 
...
## Taxonomic Group
```{r}
taxa <- select(wg, title, year, taxonomic.group, distribution, abundance, 
...
rf <- select(wg, c(title, data.type...what.where.when.only:
...
if(diag) {
  pander(table(wg$results.type...descriptive.only), caption = "Descriptive Only Results (need at least 10 events/non evenst per predictor)")
}
...
```{r results_by_data}
wg$quantitative <- wg$results.type...hypothesis.testing == T |
  wg$results.type...inference == T | wg$results.type...prediction == T

long_wg <- select(wg, title, quantitative, data.type...what.where.when.only:
                     data.type...detection...non.detection, 
...
long_wg$quantitative <- gsub("TRUE", "inference/prediction", as.character(long_wg$quantitative))
long_wg$quantitative <- gsub("FALSE", "descriptive", as.character(long_wg$quantitative))
kable(table(long_wg$data_type, long_wg$quantitative))

```

```{r, echo = T}
if(diag) {
  table(wg$data.type...what.where.when.only)
  table(wg$data.type...abundance)
  table(wg$data.type...detection...non.detection)
  table(wg$data.type...sampling.effort.reported)
  table(wg$data.type...organized.data.collection.scheme)
  table(wg$data.type...visit.specific.covariates)
  table(wg$data.type...physical.specimen)
}
```
