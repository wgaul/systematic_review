...
# make names prettier
spat_extent$data_type <- gsub("data.type...", "", spat_extent$data_type)
...
wg$log_spat_extent <- gsub(" km\\^2", "", wg$spatial.extent.of.study)
wg$log_spat_extent <- log(as.numeric(as.character(wg$log_spat_extent)))
...
median_fun <- function(x, indices) {
  ...
  x_b <- x[indices]
  median(as.numeric(x_b), na.rm = TRUE)
}

spat_time_df <- select(wg, c(spat_extent, 
        data.structure...sampling.effort.known, 
        data.structure...non.detection,
        data.structure...organized.data.collection.scheme, 
        data.type...visit.specific.covariates, 
        data.structure...multiple.datasets.integrated.for.analysis)) %>%
  filter(!is.na(spat_extent))

spat_time_boot_array <- array(NA, dim = c(nrow(spat_time_df), 
                                          ncol(spat_time_df), 
                                          n_perm))
# draw bootstrap samples 
stop("I should use boot function instead so I can caluclate bca easier.")
for(i in 1:n_perm) {
  spat_time_boot_array[, , i] <- as.array(as.matrix(
    spat_time_df[sample(1:nrow(spat_time_df), 
                        replace = TRUE), ]))
}
```
...
spat_extent_cap <- figs(name = "spat_extent_cap", 
                        "The spatial extent covered by studies was significantly different for studies using different types biological records data.  This plot shows the observed temporal extent of studies. Note the log-transformed y-axis.")
```
...
```{r spat_extent_results_plot, fig.cap=spat_extent_cap}
spat_extent_plot <- ggplot(
  data = spat_extent, 
  aes(y = spatial.extent.of.study, 
      x = factor(data_type, 
                 levels = c("sampling.effort.known", 
                            "non.detection", 
                            "organized.data.collection.scheme", 
  ...
  scale_y_log10() + 
  xlab("Data Type") + 
  ylab("Spatial Extent of Study (km^2)") + 
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 35, vjust = 0.9, hjust = 1)) 
...
The use of data types differed significantly between study questions (*p* = `r round(dt_pd_interaction_test$"Pr(>Chi)"[2], 2)` for chi-squared likelihood ratio test of a null hypothesis that a Poisson regression independence model with main effects for data type and ecological study focus (but no interaction term) model fit the data as well as a saturated model.  In the independence model, two cells had residuals with an absolute value bigger than two (Fig. 4) - the cell for the number of studies investigating abundance using multiple biological datasets (residual = -3.7), and the cell for the number of studies investigating species distributions using multiple biological records datasets (residual = 3).  Residuals were mostly positive for combinations of structured data types and study questions about abundance, and negative for combinations of structured data types and species distribution study questions (Fig. 4).  This suggests that abundance studies use structured data types more often than would be expected if data type and study question were not related, and distribution studies use structured data types less often than would be expected if study question and data type were not related. 
