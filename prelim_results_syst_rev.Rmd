...
To investigate whether the data type affects the analysis approach, we used a random permutation procedure to test the null hypothesis of no difference in the types of data used with different analysis approaches.  We identified the analysis approach of each study as inference, prediction, or descriptive analyses only.  The data type categories we tested based on *a-priori* expectations of importance were (in order of expected importance): sampling effort; non-detection; organized monitoring scheme; visit-specific covariates; and multiple datasets integrated for analysis.  In the ranom permutation test, we broke the association between data type and analysis approach by randomly shuffling the analysis approach results.  We preserved associations between analysis approaches by preserving the rows (and therefore associations) within the analysis approach variables, and we preserved associations between data types by preserving the rows (and therefore associations) within the data type variables (Fig. S??).  We performed 10,000 permutations, and for each permutation counted the number of studies using each combination of analysis approach and data type. We calculated empirical p-values by finding the proportion of the 10,000 permuted cell count values that were smaller than the observed value if the observed value was below the median of the permuted values, or above the observed value if the observed value was greater than the median of the permuted values.  We then doubled this proportion to make a two-sided empirical p-value giving the probability of a cell count more extreme than the observed cell count given the null hypothesis.  Small values of the empirical p-value for a cell indicate that the observed number of studies in that cell is unlikely if there was no relationship between that data type and that analysis approach.  
...
# make empty template df to hold p-values later
...
```{r data_type_analysis_resids_table}
kable(dt_an_df[abs(dt_an_df$resids) > 1, ], 
      col.names = c("Analysis approach", "Data type", "Observed number of studies", 
                 "Predicted number of studies", "Residuals (on the scale of the linear predictors)"), digits = 1)
```
...
ggplot(dt_an_df, 
       aes(x = factor(data_type, 
           ...
           fill = factor(data_type, 
                         levels = c("data.structure...sampling.effort.known", 
                                    "non.detection", 
                                    "organized.data.collection.scheme", 
                                    "multiple.datasets.integrated.for.analysis", 
                                    "visit.specific.covariates"), 
...
perm_p_vals_long <- gather(perm_p_vals, key = "inference_method", 
                           value = "p_value", 2:4)
ggplot(data = perm_p_vals_long, aes(x = inference_method, y = data_type)) + 
  geom_tile(aes(fill = p_value)) + 
  geom_text(aes(label = round(p_value, 2))) + # write the values
  ...
  labs(fill = "empirical\np-value") + 
  ggtitle("Empirical p-values from\npermutation test")

perm_resids_long <- gather(perm_resids, key = "inference_method", 
                           value = "residual", 2:4)
ggplot(data = perm_resids_long, aes(x = inference_method, y = data_type)) + 
  geom_tile(aes(fill = residual)) + 
  geom_text(aes(label = round(residual, digits = 1))) + # write the values
  ...
  theme(panel.grid.major.x=element_blank(), #no gridlines
        ...
        axis.text.y = element_text(angle=45)) + 
  labs(fill = "observed -\nexpected") + 
  ggtitle("Residuals from permutation test")
```

```{r print_data_type_analysis_method_heatmap_table_poisson}
ggplot(data = dt_an_df, aes(x = analysis_approach, y = data_type)) + 
...
# Do I need to fix overdispersion, or should I just interpred the residuals?
```
