...
### Methodological Focus (TODO)
**Jon, how to use 'across' vs. 'between' in stats-y talk?**
...
To test for differences in the the number of studies focusing on methodology across the different study focuses, we used logistic regression with six binary predictor variables indicating the specific biological or ecological focus(es) of the study and a binary response variable indicating whether the study addressed a methodological question.  To evaluate whether the assumptions for using logistic regression were met, we evaluated the number of samples per predictor variable, checked that all levels of predictor variables had more than 5 cases, **checked for multicollinearity??**, performed a Hosmer-Lemeshow goodness of fit test, looked at the confusion matrix, and identified influential points using Pearson and deviance residuals and leverage.  

(Could lump sp.richness and diversity if counts of diversity remain small)
  ...
  method_null_misclass_rate <- (method_null_conf_matrix["1", "FALSE"] + 
                             tryCatch(
                               ...
                               error = function(x) 0))/length(
                                 method_mod_null$y)
  method_diags <- LogisticDx::dx(method_mod)
}
```
...
We asked whether the data type affects the analysis approach.
...
# fit saturated model
dt_an_mod_full <- glm(n ~ analysis_approach + data_type + 
                        analysis_approach*data_type, data = dt_an_df, 
                      family = poisson())
# fit model assuming independence between analysis approach and data type
dt_an_mod_ind <- glm(n ~ analysis_approach + data_type, 
                     data = dt_an_df, family = poisson())
dt_an_mod_indepence_test <- anova(dt_an_mod_full, dt_an_mod_ind, test = "Chisq")
```
...
kripp_sum_cap <- figs(name = "kripp_sum_cap", 
                      paste0("Reader agreement measured using Krippendorff's alpha, for variables for which the sample size is large enough to estimate Krippendorff's alpha.  Dotted lines show Krippendorff's alpha values of 0.667 and 0.8, which are recommended minimum and preferred values at which variables can be relied upon for analysis."))
```
...
For variables for which an adequate number of studies have been double coded to allow estimation of Krippendorf's alpha, most varibales show that coding is reliable enough to be used in analysis, at least for drawing tentative conclusions (`r figs("kripp_sum_cap", display = "cite")`).  However, the number of articles that have been evaluated by two readers is too small for reliable estimation of Krippendorff's alpha for some variables for which the smallest class is particularly uncommon (`r tabs("kripp_samp_size_cap", display = "cite")`).
...
**Assumptions**
```{r print_method_assumptions}
if(diag_present) {
  # assumptions for logistic regression for methodology focus by study focus
  print("Need at least 10 events/non-events per predictor.  ")
  print(kable(data.frame(table(wg$methodology.development.or.analysis))))
  print("Need no sparse cells (no cells < 5)")
  data.frame(select(wg, response.variable...phenology, trends.over.time, 
  ...
  method_mod_vif # VIF should be < 10 to avoid multicollinearity
  method_mod_hlTest # H-L test, H_0: Model fits the data well
  method_conf_matrix # confusion matrix with threshold 0.5
  # Does the model with predictors have lower misclassification rate than null?
  method_misclass_rate # misclassification rate for full model
  method_null_misclass_rate # misclassification rate for null model
  hist(method_diags$Pr, main = "Pearson residuals for methodology\nby study focus logistic regression")
  print("Articles with Pearson residuals > 2 for logistic regression predicting methodology by study focus: ")
  wg$title[abs(method_diags$Pr > 2)]
  hist(method_diags$dr, main = "Deviance residuals for methodology\nby study focus logistic regression")
  print("Articles with deviance residuals > 2 for logistic regression predicting methodology by study focus: ")
  wg$title[abs(method_diags$dr > 2)]
  hist(method_diags$h, main = "Leverage for methodology\nby study focus logistic regression")
  print("Articles with leverage > 2 for logistic regression predicting methodology by study focus: ")
  wg$title[abs(method_diags$h > 2)]
}
```

**Logistic regression predicting methodology focus based on ecological focus:**
```{r print_method_focus_model}
anova(method_mod, method_mod_null, test = "Chisq")
```

**TODO: graph of methodology by study focus**


### Specific Study Focus
**Drop this and use only 'Methodological Focus' analysis above?
...
**TODO:** Jon - stats? interaction will not be very interesting b/c some focuses just don't make sense in community- or individual species-only studies.  But could test whether there are differences for the subset of focuses that do make sense for all - abundance, temporal trends, and methodology.
...
if(diag_present) {
  ...
  print("Table of counts for data type by study question paradigm\n")
  print(kable(dt_an_df_wide))
}
```

```{r data_type_analysis_cap}
data_type_analysis_cap <- figs(name = "data_type_analysis_cap", 
                            "Analysis approaches used with different types of biological records data.")
```
