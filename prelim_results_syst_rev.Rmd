...
for(i in 2:ncol(er)) {
  dbl_coded_list[[i-1]] <- data.frame(matrix(
    data = c(er[, i][[1]], mult_coded_wg[, i][[1]]), 
...
kripp_summary_long <- data.frame(
  set = rep("all_dbl_coded", length(kripp_all_dbl_coded)), 
...
alpha_df <- data.frame(
  set = rep("all", length(names(kripp_all_dbl_coded))), 
  variable = names(kripp_all_dbl_coded),  
