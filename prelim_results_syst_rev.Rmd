...
# which variables were unreliable (i.e. sample size big enough but alpha low)
prob_of_values[prob_of_values$prob_small_class >= 0.14 & 
                 prob_of_values$kripp_alpha <= 0.67, ]
```

`r nrow(prob_of_values[prob_of_values$prob_small_class >= 0.14 & prob_of_values$kripp_alpha > 0.67, ])` variables met our criteria for inclusion in further analyses (Krippendorff's alpha greater than 0.67 and relative frequency of at least 0.14 for the least common category).  `r nrow(prob_of_values[prob_of_values$prob_small_class >= 0.14 & prob_of_values$kripp_alpha <= 0.67, ])` variables had relative frequencies of the least common category high enough to estimate alpha, but had alpha values less than or equal to 0.67 and were therefore deemed unreliable.  
...
# add observed counts to residuals data frame
dt_pd_df_temp <- dt_pd_df # make a copy of dt_pd_df for modifying column names
colnames(dt_pd_df_temp)[colnames(dt_pd_df_temp) == "n"] <- "count" # rename column
colnames(dt_pd_df_temp)[colnames(dt_pd_df_temp) == "study_focus"] <- "study_question"
perm_sq_resids_long <- left_join(perm_sq_resids_long, dt_pd_df_temp, 
...
perm_resids_long <- gather(perm_resids, key = "analysis_approach", 
                           value = "residual", 2:4)
# add observed counts to residuals data frame
dt_an_df_temp <- dt_an_df # make a copy of dt_an_df for modifying data_type column
dt_an_df_temp$data_type <- gsub("\\.", " ", dt_an_df_temp$data_type)
colnames(dt_an_df_temp)[colnames(dt_an_df_temp) == "n"] <- "count" # rename column
perm_resids_long <- left_join(perm_resids_long, dt_an_df_temp, 
...
ggsave("Fig1.eps", temp_extent_plot_ridge + 
       ...
       device = "eps")
ggsave("Fig2.eps", response_var_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "eps")
ggsave("Fig3.eps", data_type_study_question_perm_plot + 
         theme(text = element_text(size = t_size_save), 
  ...
  geom_text(aes(label = paste0(round(residual, digits = 1), "\n(", count, 
       ...
       width = 25, height = 25, units = "cm", 
       device = "eps")
ggsave("Fig4.eps", data_type_analysis_approach_plot + 
         theme(text = element_text(size = t_size_save), 
        legend.key.height = grid::unit(1.5, "cm")) + 
  geom_text(aes(label = paste0(round(residual, digits = 1), "\n(", count, 
                               ")\n",  sig_symb)), 
...
ggsave("Gaul Figure 1.jpg", temp_extent_plot_ridge + 
       ...
       device = "jpg")
ggsave("Gaul Figure 2.jpg", response_var_plot + 
         theme(text = element_text(size = tsize_manuscript)), 
       width = 11, height = 11, units = "cm", 
       device = "jpg")
ggsave("Gaul Figure 3.jpg", data_type_study_question_perm_plot + 
         theme(text = element_text(size = tsize_manuscript), 
       ...
       width = 11, height = 11, units = "cm", 
       device = "jpg")
ggsave("Gaul Figure 4.jpg", data_type_analysis_approach_plot + 
         theme(text = element_text(size = tsize_manuscript), 
               legend.key.height = grid::unit(1, "cm")) + 
  geom_text(aes(label = paste0(round(residual, digits = 1), "\n(", count, 
                               ")\n",  sig_symb)), 
