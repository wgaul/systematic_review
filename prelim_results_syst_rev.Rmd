...
We expected the temporal and spatial extent covered by studies to be smaller for studies using more structured or "rich" data types compared to studies using less structured data.  We tested whether the temporal extent of studies differed based on the data type by fitting a linear regression with the natural-log transformed temporal extent of studies (in years) as the response variable.  We used five binary predictor variables indicating whether the study used at least some data that had the following characteristics or information: known sampling effort; explicit non-detection data; data from an organized monitoring scheme; visit-specific covariates; or multiple different biological records datasets used for one analyses.  Although some of the predictor variables were correlated (Fig. S??), we kept all variables in the regression model because we were interested in the effect of each additional type of information *after accounting for other information types*.  For example, we were interested in the additional effect of having non-detection data after adjusting for whether the data came from an organized monitoring scheme.  We assessed collinearity of predictor variables using variance inflation factors in order to determine if collinearity is likely to cause additional uncertainty in the the coefficient estimates for the effect of each data type (Fox 2016). Because the data types were not mutually exclusive, each data type category was treated as an individual binary variable, rather than being treated as a factor level of a categorical variable.  We assessed the significance of data types for predicting temporal extent by using a likelihood ratio test to compare a model containing main effects (but no interactions) for all data types to an intercept only model.  We then assessed the significance of each individual data type by comparing the model with all main effects to a reduced model in which the variable representing the data type being tested had been removed.
...
temp_extent_preds <- data.frame(
  "data.structure...sampling.effort.known" = c(T,F,F,F), 
  "data.structure...non.detection" = c(F, T, F, F), 
  "data.structure...organized.data.collection.scheme" = c(F, F, T, F), 
  "data.structure...multiple.datasets.integrated.for.analysis" = c(F,F,F,T))
...
We asked whether methodological development is more active for some types of ecological questions than for others.  To test for differences in the proportion of methodology studies across the different biological and ecological study focuses, we used logistic regression with six binary predictor variables indicating the specific biological or ecological focus(es) of the study and a binary response variable indicating whether the study addressed a methodological question.  To evaluate whether assumptions were met for using logistic regression, we evaluated the number of samples per predictor variable, checked that all levels of predictor variables had more than five cases, checked for multicollinearity using the variance inflation factors [@Fox1992], performed a Hosmer-Lemeshow goodness of fit test, and identified influential points using Pearson and deviance residuals and leverage.  
...
```{r spat_extent_cap}
spat_extent_cap <- figs(name = "spat_extent_cap", 
                        "The spatial extent covered by studies was not significantly different for studies using different types biological records data.  Black cirles show the mean spatial extent of studies using each type of data and lines show 95% bootstrap CI around the mean.")
```
...
The temporal extent of studies differed significantly for studies using different types of biological records data (anova of a linear regression model using data type to predict the natural log of the temporal extent of studies (in years) compared to a model that did not use data type $F_{`r format(round(m_time_full_anova[2, "Df"], digits = 2), scientific = F)`, `r format(round(m_time_full_anova[2, "Res.Df"], digits = 2), scientific = F)`}$ = `r format(round(m_time_full_anova[, "F"][2], digits = 2), scientific = F)`, *p* `r if(m_time_full_anova[6][[1]][2] >= 0.0001) {format(round(m_time_full_anova[6][[1]][2], digits = 4), scientific = T)} else "< 0.0001"`, Adjusted $R^2$ = `r format(round(time_summary$adj.r.squared, digits = 2), scientific = F)`, Fig. 1).  Studies that used multiple different biological records datasets covered significantly longer temporal extents than studies that did not use multiple different datasets after accounting for other data types ($F_{1, 196}$ = `r round(anova(indiv_mods_time$m_no_multData, time_lm_full)$F[2], digits = 2)`, *p* < 0.0001, Fig. 1).  Studies that used data from an organized monitoring scheme covered significantly shorter temporal extents than studies that did not use data from an organized monitoring scheme after accounting for other data types ($F_{1, 196}$ = `r round(anova(indiv_mods_time$m_no_scheme, time_lm_full)$F[2], digits = 2)`, *p* = `r round(anova(indiv_mods_time$m_no_scheme, time_lm_full)$'Pr(>F)'[2], digits = 2)`). The expected temporal extent covered by a study that used multiple biological records datasets was `r round(exp((time_lm_full$coefficients[1] + time_lm_full$coefficients[5])), digits = 1)` years (95% CI: [`r round(temp_extent_preds$pred_years_lowerCI[4], digits = 1)`, `r round(temp_extent_preds$pred_years_upperCI[4], digits = 1)`]) when the data did not have any of the other data types we recorded.  The expected temporal extent of a study that used data from an organized monitoring scheme was `r round(exp(time_lm_full$coefficients[1] + time_lm_full$coefficients[4]), digits = 1)` years (95% CI: [`r round(temp_extent_preds$pred_years_lowerCI[3], digits = 1)`, `r round(temp_extent_preds$pred_years_upperCI[3], digits = 1)`]) when the data did not have any of the other data types we recorded.

We did not detect a significant effect of sampling effort data ($F_{1,196}$ = `r round(anova(indiv_mods_time$m_no_effort, time_lm_full)$F[2], digits = 2)`, *p* = `r round(anova(indiv_mods_time$m_no_effort, time_lm_full)$'Pr(>F)'[2], digits = 2)`) or non-detection data ($F_{1,196}$ = `r round(anova(indiv_mods_time$m_no_nonDet, time_lm_full)$F[2], digits = 2)`, *p* = `r round(anova(indiv_mods_time$m_no_nonDet, time_lm_full)$'Pr(>F)'[2], digits = 2)`) on the temporal extent of studies.  However, pairs plots and variance inflation factors show that the "sampling effort" and "non-detection" data types are strongly correlated with each other, and both are correlated less strongly with the "organized monitoring scheme" data type (Fig. S?, Tab. S1), suggesting that we may have been unable to dis-entangle the effects of each of these correlated variables with our current sample size (Fox 2016, p. 341-2).  We removed from the analysis `r length(which(is.na(wg$temp_extent)))` studies for which the coder could not determine the temporal extent.
...
```{r spat_extent_bootCI_plot, fig.cap=spat_extent_cap}
spat_boot_df <- data.frame(matrix(nrow = length(data_types), 
                                  ncol = 4))
colnames(spat_boot_df) <- c("data_type", "mean", 
                             "l_95ci", "u_95ci")
...
resp_var_plot_cap <- figs(name = "resp_var_plot_cap", 
                     "The proportion of studies analyzing different ecological responses.  Points show proportion of studies, vertical lines show 95% bootstrap confidence intervals.  Some studies performed multiple analyses and used multiple different response variables, so proportions in this plot add up to more than one.")
```
...
method_study_focus_cap <- figs(name = "method_study_focus_cap", 
                               "The coefficient estimates (black circles) from a logistic regression testing whether the probability of a study developing or testing methodology depended on the biological or ecological response variable.  We found no difference in the probability of studies addressing methodological questions based on the biological or ecological response being studied.  All confidence intervals (vertical lines) for regression coefficients overlapped zero.")
```

The overall model predicting the probability of a study addressing a methodological question based on the ecological focus of the study was not significant (intercept-only model residual deviance = `r round(method_chisq$'Resid. Dev'[1], digits = 1)`, full model residual deviance = `r round(method_chisq$'Resid. Dev'[2], digits = 1)`, Chi-squared test *p* = `r round(anova(method_mod, method_mod_null, test = "Chisq")[2, 5], 2)`, Fig. 3).  
...
meth_df$variable <- c("abundance", "distribution", "phenology", 
                      "richness", "time", "other")
...
The permutation test suggested that data type and study question were not independent (permutation-based significance level of the chi-squared test statistic for the observed data = `r chis_perm_study_question_pvalue` with `r n_perm` permutations).  Permutation test residuals and significance levels for each combination of data type and study question showed that residuals were generally large, positive, and significantly different from zero for combinations of structured data types and studies of species abundances, suggesting that those data types were used more frequently for abundance studies than would be expected if there were no relationship between data type and analysis approach. Residuals were large, negative, and highly significantly different from zero for combinations of more structured data types and studies of species distributions (Fig. 4?), suggesting that those rich data types are used for distribution studies less often than would be expected if there was no relationship between data type and analysis approach. The absolute value of residuals and significance levels for combinations of all data types with studies of phenology, species richness and "other" study questions generally indicated that the use of data types for those study questions did not differ strongly from what would be expected if there were no relationship between data type and study question.  The residuals and significance levels suggested that multiple datasets were used less often for abundance studies and more often for distribution studies (and perhaps for studies of "other" questions) than would be expected if there was no relationship between data type and study question.  Both the absolute magnitude of residuals and the significance levels suggested that the observed use of multiple datasets for abundance and distribution questions differed less strongly from the null expectation than did combinations of structured data types with those study questions.  

```{r data_type_study_question_perm_cap}
data_type_study_question_perm_cap <- figs(name = "data_type_study_question_perm_cap", 
                               "Residuals from a permutation test of independence between data type and biological or ecological study question show that more structured types of data (non-detection, sampling effort, and data from organized monitoring schemes) are used more frequently to study species abundances and less frequently to study species distributions than would be expected if data type and study question were not related.  When multiple different biological records datasets were integretad for a one study, that study was more likely to study species distributions and less likely to study abundance than would be expected if there was no relationship between data type and analysis approach. The data types used for studies of phenology and species richness did not differ strongly from what would be expected if there was no relationship between data type and analysis approach.  Residuals for each cell were calculated as the difference between the observed number of studies in each cell and the mean of the number of studies in the same cell in 10,000 permuted datasets.  Values in each grid cell are presented as: residual (permutation significance level).  Small values of permutation significance level indicate that the value in that cell would be unlikely if there were no relationship between data type and study question.  Colors indicate direction and size of residuals.")
```


```{r print_data_type_study_question_heatmap_table_permutation, fig.cap=data_type_study_question_perm_cap}
...
```{r data_type_analysis_method_perm_cap}
data_type_analysis_method_perm_cap <- figs(
  name = "data_type_analysis_method_perm_cap", 
  "Residuals from a permutation test of independence between data type and analysis method show that more structured types of data (non-detection, sampling effort, and data from organized monitoring schemes) are analysed more frequently with statistical inference and less frequently with purely descriptive methods than would be expected if data type and analysis method were not related.  When multiple different biological records datasets were integretad for a one study, that study was more likely to use purely descriptive analyses and less likely to use statistical inference than would be expected if there was no relationship between data type and analysis approach. The data types used for predictive analyses did not differ strongly from what would be expected if there was no relationship between data type and analysis approach.  Residuals for each cell were calculated as the difference between the observed number of studies in each cell and the mean of the number of studies in the same cell in 10,000 permuted datasets.  Values in each grid cell are presented as: residual (permutation significance level).  Colors indicate direction and size of residuals.")
```

```{r print_data_type_analysis_method_heatmap_table_permutation, fig.cap=data_type_analysis_method_perm_cap}
# get permutation significance levels
...
```{r, prox_src_tab_cap}
prox_src_tab_cap <- tabs(name = "prox_src_tab_cap", 
                                   "Proximate data providers used by five or more studies in this review.")
```
...
Studies in this review obtained their data from at least `r nrow(prox_src_df)` different proximate data sources.  The number of studies using each data source was strongly right-skewed, with many data sources that were used by only one study, and relatively few data sources that were used by many studies (`r tabs("prox_src_tab_cap", display = "cite")`, Tab. S??).  Among the most frequently used data sources were both national and international data aggregators (e.g. the UK-focused National Biodiversity Network, the Irish National Biodiversity Data Centre, and the Global Biodiversity Information Facility) and individual organizations that manage both the initial collection and the subsequent presentation of data (e.g. the British Trust for Ornithology and the UK Butterfly Monitoring Scheme, `r tabs("prox_src_tab_cap", display = "cite")`).  Of the `r nrow(wg)` studies in this review, `r length(which(used_top_10_providers == T))` (`r round((length(which(used_top_10_providers == T)) / length(used_top_10_providers)) * 100, digits = 1)`%) obtained at least some of their data from one of the ten most commonly used proximate data providers (`r tabs("prox_src_tab_cap", display = "cite")`).
...
`r tabs("prox_src_tab_cap", display = "full")`
...
## Uncommon uses of biological records
[biological records as a predictor variable or in a facilitative role]
...
# Discussion
By far the most common use of British and Irish biological records found by this review was for studying species distributions.  For this and all other study questions, the proportion of studies that were methodological (developing or testing new methods) was relatively low (??%, section ??), suggesting that, by and large, the research community views existing methods for analyzing biological records as adequate and ready to apply to study biological and ecological questions. We did find evidence that the type of data constrains the analysis approach (Fig ??), with researchers apparently avoiding using statistical inference when they used multiple different biological datasets (or perhaps avoiding using multiple different datasets when they did statistical inference), and using inference more commonly when the data came from organized monitoring schemes, or contained survey effort information or explicit non-detection data.  This suggests that integrating multiple different biological records datasets for a single study comes at a cost of data quality, or perhaps a cost in terms of the ease with which data from multiple different sources can be integrated into a single model.  However, studies using multiple different datasets covered greater temporal extents (Fig ??).  Studies of species distributions were also more likely to use multiple different biological records datasets than studies of other questions (Fig ??), suggesting that researchers felt that using multiple different datasets added important information to distribution studies, or perhaps that integrating data from multiple sources into a single model is easier for distribution models than for models used for some other study questions.  

It is possible that intensive methodlological work may result in a suite of tools and methods for applying a particular type of data to a particular problem. The more frequent use of multiple different biological records datasets for distribution studies compared to studies of other questions could be the result of a previous period of methodological development that focused on applying diverse data to distribution models. Our review would not be able to identify such a situation.  However, we did expect to find that methodological work was more active for some types of questions, probably questions for which biological records have been less commonly used but for which there is potential for important contributions from biological records.  However, we did not detect any difference in the probability of a study addressing a methodological question based on the biological or ecological focus of the study (Fig ??).  There may be opportunities for more methodological work to "unlock" different types of biological records to study questions for which they are currently used less frequently, for example developing methods for using multiple different biological records datasets for abundance studies (Fig. 5).

Very few studies in our review used cross-validation or tested models on independent test data sets (Section ??).  This is suprising and concerning, given that the overall picture that emerged of the use of biological records is that existing methods and data seem to be viewed as "ready to use" by the reserach community (i.e. a large majority of studies are addressing biological or ecological questions rather than testing or developing methodology).  It is well known that tests of model performance are overly optimistic when data have spatial, temporal, or other correlation structures and model testing uses the same data that is used for model fitting (Roberts et al. 2017, El Gabbas??, Elith??, McGill?, others??).  Over half the studies in this review studied species distributions (Fig. 3), but of those, only ?? (??%) used cross-validation or tested using independent data. Thus, the sense that the research community has that biological records and existing methods can be successfully used for modelling species distributions may be based on a large body of work that is not accurately assessing the true performance of models (cite roberts or something else?? McGill??).  McGill?? found that tests over-estimated AUC by ?? when using random compared to spatial block cross validation.  Solutions to the problem of testing models, including spatial block cross-validation and testing on independent datasets (Elith et al. ??) are now widely documented and are available in software packages (cite blockCV).  It is imperative that future studies of species distributions using biological records (or any data, for that matter), test models against independently collected data or using spatial block cross-validation.  For species distribution modeling, the results of this review suggest one possible solution: multiple different biological records datasets are used more commonly for distribution studies than for other study questions - perhaps one or a few of these different datsets could be withheld from model fitting and used only for model testing.  All opportunistically collected biological records data will contain some biases and gaps, but those biases and gaps are likely to be at least slightly different from one dataset to another.  So using different datasets biological records datasets for model fitting and testing, combined with spatial block cross-validation, offers the possibility of testing the ability of models to generalize to data in new spatial locations and data collected using at least somewhat different methods, perhaps by different people.  This approach is possible for birds in the British Isles, for which biological records data are available from both semi-structured (e.g. Garden Bird Survey) and unstructured (e.g. BirdTrack) schemes run by the British Trust for Ornithology and Birdwatch Ireland, and from eBird.  Similarly, for butterflies, data are available from both the Butterflies for the New Millenium project and from ??; one of those datasets could be used to fit models while the other could be used for model testing, as was done by ??cite a study that did this??.  
...
We only found one study in this review that used data from the bird song audio archive site xeno-canto, which was surprising because xeno-canto is more explicitly oriented towards collecting and providing biological sounds, unlike more general media sharing websites like soundcloud and vimeo.  It is possible that our search terms failed to return studies that used data from xeno-canto if those studies were framed more as studies of bird vocalizations rather than as studies of biological records.  Nevertheless, data from sites like xeno-canto share much in common with other biological records datasets in which basic "what, where, when" information about species is augmented with additional information.  The opportunistic nature of data in xeno-canto means that analyses using those data likely face challenges similar to those faced by analyses of other opportunistic or semi-structured biological records datasets.  

We were not able to estimate the reliability of our codings about whether studies tested their statistical models using cross-validation and/or independently collected test data, because there were too few studies testing with those approaches.  However, the rarity of testing using cross-validation and/or independent data is an interesting results, even if we cannot analyze it robustly because of uncertainty about our coding reliability.  Testing statistical models using the same data that was used for model trianing is known to give overly optimistic measures of model performance (cite).  It is concerning that studies using biological records data seem to be either failing to test on independent data or failing to clearly report that they tested on independent data.  The opportunistic nature of biological records datasets means that the data may not represent a random sample from the population of interest in a study.  As such, it seems particularly important that conclusion drawn from models trained with biological records data are tested on independent or semi-independent data (e.g. using cross-validation).  Even if the independent test dataset has biases similar to the training data, it at least provides a minimum standard for determining whether results can be extrapolated beyond the peculiarities of the training data.  ??Cross-validation is commonly used to assess prediction performance in machine learning, and there are packages that implement cross-validation specifically for species distribution modelling.  Cross-validation can also potentially be used with more traditional statistical methods (e.g. linear regressions) to test how well fitted models agree with data that were not used in model fitting.??

Digital record submission and storage has increased the ease with which vouchers of specimens can be stored and accessed.  Digital photos, audio recordings, and videos can all be used as digital voucher specimens to both confirm the taxonomic identity of species recorded and to provide additional information that may be of interest to reserachers.  Sites that collate recordings of bird songs (e.g. xeno-canto and the Macaulay Library of Natural Sounds) now hold ??hundreds of thousands?? of recordings of British and Irish birds, and eBird, one of the worlds largest platforms for collecting biological records, now allows observers to upload photos, audio recordings, and videos?? with species checklists. Given the large and rapidly growing archive of digital voucher specimens (??number and growth of eBird audio or video files??) and the relatively infrequent use of these data in studies in this review, the analysis of the digital vouchers associated with biological records may be a rich area for new research (cite some studies that do this).  The challenges associated with studying e.g. the distribution of bird song dialects (??cite??) or the distributions of morphological characteristics (??cite??) using biological records with digital vouchers are likely similar to the challenges (and the solutions to those challenges) associated with studying species occurrence distributions using more traditional "what, where, when" biological records.  It is possible that our search terms did not retrieve studies that used biological records with digital vouchers if those studies did not include words like "biological records" or "citizen science".  

Biological records data were used as both response and predictor variables in studies in this review.  By far the most common way to use biological records was as a response (`r length(which(wg$biological.records.as.response.variable == T))` of `r nrow(wg)` studies), for example when using environmental predictor variables to estimate the probability of occurrence (measured with biological records) of a species in species distribution modelling (?cite a study?). However, biological records were used as a predictor in multiple studies. For example, the recorded presence of wild mammals and seabirds was used as one of many predictors to estimate the spatial distribution of air pollutant emissions (ammonia, methan, and nitrous oxide) (Tomlinson, Carnell, Tang, Sutton & Dragosits 2017).
...
kripp_sum_cap <- figs(name = "kripp_sum_cap", 
                      paste0("Reader agreement measured using Krippendorff's alpha, for variables for which the sample size was large enough to estimate Krippendorff's alpha.  Dashed and dotted horizontal lines show Krippendorff's alpha values of 0.8 and 0.67, which are recommended preferred and minimum values at which variables can be relied upon for analysis."))
```
...
We asked whether different types of biological records data are used to study different types of questions.  We initially fit Poisson regression models of the number of studies with each combination of data type and biological or ecological study focus, fitting both a saturated model and an independence model with only main effects.  The Poisson independence model (no interatctions) was overdispersed (ratio of residual deviance to residual degrees of freedom = `r round(dt_pd_mod_ind$deviance/dt_pd_mod_ind$df.residual, digits = 2)`), so in the main text we opted to use a random permutation test which made no assumptions about the distribution of the data or residuals.  Here, we report the results and model residuals from the Poisson independence model (Fig. S??), which showed the same general pattern in the direction and size of residuals as the randomization test (Fig. ??), though there were some differences in relative sizes of residuals between the permutation and Poisson approaches.  Overall, the consistent pattern in the direction of residuals suggested that the interpretation of the results was robust to using different statistical tests. 
 
The use of data types differed significantly between study questions (*p* = `r round(dt_pd_interaction_test$"Pr(>Chi)"[2], 3)` for a chi-squared likelihood ratio test of a null hypothesis that a Poisson regression independence model with main effects for data type and ecological study focus (but no interaction term) model fit the data as well as a saturated model.  In the independence model, two cells had deviance residuals with an absolute value bigger than two (Fig. 4) - the cell for the number of studies investigating abundance using multiple biological datasets (residual = -3.5), and the cell for the number of studies investigating species distributions using multiple biological records datasets (residual = 2.8).  The residuals for those two cells were not as big in the permutation test reported in the main text, but the direction of the residuals was the same.  Deviance residuals from the Poisson model were generally positive for combinations of structured data types and study questions about abundance, and negative for combinations of structured data types and species distribution study questions (Fig. S??). This was in aggreement with the permutation test (Fig ??) and suggested that abundance studies use structured data types more often than would be expected if data type and study question were not related, and distribution studies used structured data types less often than would be expected if study question and data type were not related.

```{r data_type_study_question_cap}
data_type_study_question_cap <- figs(name = "data_type_study_question_cap", 
                               "Deviance residuals from a Poisson regression model of independence between data type and study question.  The direction and size of the residuals indicated deviations from what would be expected if there were no relationship between data type and study question.  Large negative residuals indicate that there were fewer studies with that combination of data type and study question than would be expected if there were no relationship between data type and study question.  Large positive residuals indicate that there were more studies with that combination of data type and study question than would be expected if there were no relationship between data type and study question.")
```

```{r data_type_study_question_graph, fig.cap=data_type_study_question_cap}
...
data_type_analysis_cap <- figs(name = "data_type_analysis_cap", 
                            "The number of studies using different types of biological records data with different analysis approaches.  Analysis approaches are inference (e.g. *p*-values or confidence intervals), prediction (e.g. prediction of species presence or absence into unsampled areas) and description (maps, graphs, narrative descriptions, or descriptive statistics with no inference or prediction).")
```
...
```{r, prox_src_tab_cap_all}
prox_src_tab_cap_all <- tabs(name = "prox_src_tab_cap_all", 
                                   "Proximate data sources used studies in this review, and the number of studies that used each data source.")
```

`r tabs("prox_src_tab_cap_all", display = "full")`
...
The instructions for coders (Appendix 2) did not adequately explain how to code this column.  For example, for a study about a single bird species *Turdus merula*, the instructions for the "taxonomic group" column did not specify whether to record "Turdis merula", "bird", or "Turdus merula; bird".  All of those types of variants were used throughout the coding, making the results for this column uninterpretable and un-repeatable. A future review should provide more explicit instructions to coders about what level the taxonomic group should be coded at (e.g. record the Class of organisms studied).  
...
ggsave("Fig1.svg", temp_extent_plot + 
       ...
       device = "svg")
ggsave("Fig2.svg", spat_extent_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("Fig3.svg", response_var_plot + 
       ...
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("Fig4.svg", methodology_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("Fig5.svg", data_type_study_question_perm_plot + 
         theme(text = element_text(size = t_size_save), 
  ...
  geom_text(aes(label = paste0(round(residual, digits = 1), "\n(", 
       ...
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("Fig6.svg", data_type_analysis_approach_plot + 
         theme(text = element_text(size = t_size_save), 
        legend.key.height = grid::unit(1.5, "cm")) + 
  geom_text(aes(label = paste0(round(residual, digits = 1), " (", 
                               round(significance, digits = 2), ")")), 
...
ggsave("Fig1.jpg", temp_extent_plot + 
       ...
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("Fig2.jpg", spat_extent_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("Fig3.jpg", response_var_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("Fig4.jpg", methodology_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("Fig5.jpg", data_type_study_question_perm_plot + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = grid::unit(1.5, "cm")) + 
         geom_text(aes(label = paste0(round(residual, digits = 1), "\n(", 
                               round(significance, digits = 2), ")")), 
                   size = t_size_save/4), # write the values, 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("Fig6.jpg", data_type_analysis_approach_plot + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = grid::unit(1.5, "cm")) + 
  geom_text(aes(label = paste0(round(residual, digits = 1), " (", 
                               round(significance, digits = 2), ")")), 
...
ggsave("FigS1.1.svg", krip_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.2.svg", start_year_cor_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.3.svg", data_type_pairs_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
svg("FigS1.4.svg")
...
par(mfrow = c(1, 1))
dev.off()
ggsave("FigS1.5.svg", method_glm_Pr_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.6.svg", method_glm_dr_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.7.svg", method_glm_lev_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.8.svg", prop_responses_boot_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.9.svg", data_type_study_question_pois_plot + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = unit(1.1, "cm")) + 
        geom_text(aes(label = round(resids, digits = 1)), size = t_size_save/3), 
       width = 25, height = 25, units = "cm", 
       device = "svg")
ggsave("FigS1.10.svg", dt_an_pois_heatmap + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = unit(1.1, "cm")) + 
        geom_text(aes(label = round(resids, digits = 1)), size = t_size_save/3), 
       width = 25, height = 25, units = "cm", 
...
ggsave("FigS1.1.jpg", krip_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.2.jpg", start_year_cor_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.3.jpg", data_type_pairs_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
jpeg("FigS1.4.jpg")
...
par(mfrow = c(1, 1))
dev.off()
ggsave("FigS1.5.jpg", method_glm_Pr_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.6.jpg", method_glm_dr_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.7.jpg", method_glm_lev_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.8.jpg", prop_responses_boot_plot + 
         theme(text = element_text(size = t_size_save)), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.9.jpg", data_type_study_question_pois_plot + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = unit(1.1, "cm")) + 
        geom_text(aes(label = round(resids, digits = 1)), size = t_size_save/3), 
       width = 25, height = 25, units = "cm", 
       device = "jpg")
ggsave("FigS1.10.jpg", dt_an_pois_heatmap + 
         theme(text = element_text(size = t_size_save), 
               legend.key.height = unit(1.1, "cm")) + 
        geom_text(aes(label = round(resids, digits = 1)), size = t_size_save/3), 
       width = 25, height = 25, units = "cm", 
