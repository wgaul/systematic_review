...
rev$other_paradigm <- rev$community.analysis == F & 
...
# make a list to hold bootstrap results and CIs for each study questions paradigm
# paradigm_list <- list(community = list(obs = rev$community.analysis, 
#                                        boot_obj = NA, 
#                                        ci_obj = NA), 
#                       individual_sp = list(
#                         obs = rev$individual.species.analysis, 
#                         boot_obj = NA, 
#                         ci_obj = NA),
#                       other = list(obs = rev$other_paradigm, 
#                                    boot_obj = NA, 
#                                    ci_obj = NA))
...
paradigm_list <- lapply(list(community = rev$community.analysis, 
                             individual_sp = rev$individual.species.analysis, 
                             other = rev$other_paradigm), 
                        FUN = boot_paradigm)
```
...
One reader has coded `r nrow(rev)` articles and a second reader has coded `r nrow(cp_orig)` of those. 

This preliminary analysis uses the `r nrow(rev)` articles coded so far.  
...
* bio recs as predictor - Are these being used in any questions? If not, drop.
* bio recs as response
* bio recs in facilitative role?  (This is more interesting, but harder to code.  Perhaps keep for wg to code alone.  Yes, wg codes this as a "horizon scanning" type variable to keep track of examples of interesting things, but only clear cases will be discussed in article.  No need for double checking this because it won't be analyzed in any specific way.)
* testing spatial bias correction (not used for any questions I think)
* testing other data quality correction (for this and previous)
* ADD a "methodology focus" text field variable that wg fills in if "methodology development/test" is TRUE.  This would help inform questions and categories if later I decide to re-visit and code all methodology studies to explore what methodology studies are currently focusing on.  I think that for this to be useful, it would have to be more detailed than just "testing spatial bias correction."  I would want more categories to say exactly what the methodolgy is trying to do.  
