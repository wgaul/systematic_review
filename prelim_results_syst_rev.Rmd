...
rev$temp_extent[which(grepl("An assessment of bumblebee .* land use and floral.*", 
                            rev$title))] <- 0.33
...
So far, we have evaluated `r sum(!is.na(elig$qualifies))` studies for eligibility, of which `r length(which(elig$qualifies == TRUE))` are eligible for inclusion in the scoping review.
...
Use linear regression with natural-log transformed time (in years) and the following data types as predictors: detection / non-detection, abundance, sampling effort reported, organized data collection scheme, visit-specific covariates, museum.  Note that these are each individual variables - this is not a single categorical factor level "data type" variable because the variables are not mutually exclusive.  This model assumes an additive effect of each supplementary type of data.  So the base is data that is just what, where, when only (though that variable is not needed in the model, because if all other data types are FALSE then www-only will be TRUE), and the model estimates the effect of adding each additional type of supplementary data.  It is possible to add all those data types at once. 
...
Potential correlations between the following data types: abundance & detection/non-det, sampling effort, organized data collection scheme; sampling effort & det/non-det, organized data collection.
...
```{r temp_extent_lm_categorical, include = F, eval = F}
## THIS IS WRONG!!!!! Data type is not a factor variable, because it does not
## have mutually exclusive levels.  So this isn't valid.
temp_extent$log_years <- log(temp_extent$temp_extent)
factor_time_lm <- lm(log_years ~ 1 + data_type, data = temp_extent)
factor_time_resids <- rstandard(factor_time_lm)
if(diag) {
  plot(factor_time_lm) # qq plot questionable
  hist(factor_time_resids) # some residuals bigger than 2
  boxplot(factor_time_resids)
}
factor_time_summary <- summary(factor_time_lm)
factor_time_full_model_anova <- anova(factor_time_lm)
factor_time_summary
...
```{r temp_extent_lm_independent_predictors}
#### trying this the better way of using the original df and each variable as a
#### separate variable (rather than as a categorical factor variable)

## fit full model
rev$log_years <- log(rev$temp_extent)
time_lm <- lm(log_years ~ 1 + 
                data.type...detection...non.detection + 
...
time_resids <- rstandard(time_lm)
if(diag) {
  plot(time_lm)
  hist(time_resids)
  boxplot(time_resids)
}
time_summary <- summary(time_lm)
time_full_model_anova <- anova(time_lm)
time_summary
...
```{r time_data_type_test_main_effects, include = F, eval = F}
## THIS DOES THE SAME AS THE ABOVE CHUNK MORE EXPLICITLY.
# This explicitly does an anova with a null model whereas the previous 
# chunk looks at the p-value of the individual term.  For models with only
# 1 term these are equivalent, but this would be the prefered way for models
# with more terms.

## test effect of each data type predictor by comparing model with that predictor
# to a null model.
# I don't know exactly what null model to use.  I think the best null model is
# to have no data type predictors, which basically assumes wh,wh,wh data with
# no supplementary information.  Test that with a model including only the 
# data type of interest to get the effect of adding that data type alone.

## null model (what, where, when only)
m0 <- lm(rev$log_years ~ 1)

for(i in 1:length(indiv_mods_time)) { 
  print(preds[i])
  print(anova(indiv_mods_time[[i]], m0))
}
```
...
## My Questions for group meeting

* What do you think of GLM as a way to analyze this?
    * appropriate at all?
    * binary outcome? (lumping inference & prediction)
    * multi-level outcome?
    * possible correlation of predictor variables
    * no sparse cells refers to individual predictor levels, right, not predictor/outcome cells?
    
* Is there an expected count vs. observed count test for non-mutually exclusive categories of grouping variable?  (or outcome variable for that matter)
