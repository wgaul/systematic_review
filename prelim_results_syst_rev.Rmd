...
mult_coded_rev_orig <- rev[which(rev$title %in% cp_orig$title), 
                      which(colnames(rev) %in% colnames(cp_orig))]
# order rows
...
krip_all_pre_correct <- lapply(pre_correct_list, FUN = do_kripp.alpha, 
                               method = "nominal")
...
alpha_df_all <- data.frame(
  ...
  variable = names(krip_all_pre_correct),  
  alpha = sapply(krip_all_pre_correct, FUN = function(x) {x$value}),
  n_wg_T = sapply(names(krip_all_pre_correct), 
                  FUN = function(x, df = mult_coded_rev_orig) {
                    df <- data.frame(df)
                    col_vec <- df[, which(colnames(df) == x)]
                    sum(col_vec)}), 
  stringsAsFactors = FALSE,
  row.names = 1:length(names(krip_all_pre_correct))
)
...
- inference: looks pretty good.  
    - Casey missed one that had bootstrap CIs on one plot.  
    - On a second I put no inference b/c all the analyses using bio recs didn't use inference, while Casey put inference b/c there were other analyses with non-bio recs data that used inference.
- data type - museum: 
    ...
    - I think Casey missed "Evaluating promotional approaches for citizen science biological recording..." which states that all data used were records for which there was either a photo or specimen.  
...
- data type - organized monitoring scheme:  
    ...
    - For "Nature protection areas of Europe are insufficient..." Casey put TRUE for organized scheme, but I find no record of that in the article.  Seems data were agregated from many databases, but no scheme.  Don't know why he put that.  Either misunderstanding or oversight.  
